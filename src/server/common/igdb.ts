...
const queryGame = async (item: any, req: { access_token: string, client_id: string }) => {
...
const queryCover = async (gameId: string, req: { access_token: string, client_id: string }) => {
    ...
    if (body.length > 0) {
        body = body[0];
        body.url = body.url.replace("t_thumb", "t_cover_big").replace("//", "https://");
    }
    return new Promise(resolve => {
        setTimeout(() => {
            return resolve(body)
...
const queryFranchise = async (franchiseId: string, req: { access_token: string, client_id: string }) => {
...
const queryGenre = async (genreId: string, req: { access_token: string, client_id: string }) => {
